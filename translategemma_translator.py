...
class TranslateGemmaTranslator:
    ...
    def load_model(
        ...
        if model is None:
            # Re-check VRAM before attempting bf16 (INT8 may have failed)
            if torch.cuda.is_available():
                free_gb = self.get_available_vram_gb()
                if free_gb > 0 and free_gb < cfg["vram_bf16_gb"]:
                    msg = (
                        f"VRAM insuficient pentru {model_key} in BF16: "
                        f"{free_gb:.1f}GB liberi, necesita ~{cfg['vram_bf16_gb']}GB.\n"
                        f"Instaleaza corect bitsandbytes pentru INT8, "
                        f"sau alege un model mai mic (translategemma-12b / 4b)."
                    )
                    logger.error(f"[TranslateGemma] {msg}")
                    raise RuntimeError(msg)
            try:
