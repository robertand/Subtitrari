...
def check_and_install_paddleocr(use_gpu: bool = False) -> bool:
    ...
    Returnează False dacă e necesară repornirea serverului.
    """
    import subprocess
    import sys
    ...
    # 0. Verifică numpy — PaddleOCR nu merge cu numpy >= 2.0
    if np.__version__.startswith('2.'):
        logger.warning(
            f"[OCR] NumPy {np.__version__} e incompatibil cu PaddleOCR. "
            f"Se instalează numpy<2.0..."
        )
        subprocess.run(
            [sys.executable, "-m", "pip", "install", NUMPY_PIN, "--quiet"],
            check=False,
        )
        # Verifică dacă s-a instalat corect (într-un subproces)
        check = subprocess.run(
            [sys.executable, "-c", "import numpy; print(numpy.__version__)"],
            capture_output=True, text=True
        )
        ver = check.stdout.strip()
        if ver and ver.startswith('1.'):
            logger.info(f"[OCR] NumPy downgradat la {ver}. Repornește serverul pentru a activa OCR!")
        else:
            logger.error(f"[OCR] Nu s-a putut downgrada numpy (versiune: {ver}).")
        return False
    ...
    if use_gpu:
        ...
        # Ensure numpy <2.0 for Paddle compatibility
        subprocess.run(
            [sys.executable, "-m", "pip", "install", NUMPY_PIN, "--quiet"],
            check=False,
        )
